...
$('.contents-list li').on('click', function(){
  ...
  var contentTab = $('.progress-bar__item-'+id);
  $(contentTab).removeClass('hidden');
  $(contentTab).siblings('.progress-bar__item').addClass('hidden')
	$('.contents-list li').removeClass('current');
	$(this).addClass('current')	;
});
...
$('.video-preload').on('click', function(ev){
   $(this).parents(".item-responsive").find(".video-perw").hide();
   $(this).parents(".item-responsive").find(".video")[0].src += "&autoplay=1";
    ev.preventDefault();
})
...
//progress
  $(window).scroll(function(){
    var toTop = $(".progress-bar").offset().top + (-300)
...
if ($("#soft-slider-rete").length){
  ...
  reteSlider3.mount();
  reteSlider4.mount();
reteSlider.on( 'move', function () {
  reteSlider1.go(reteSlider.index );
...
reteSlider1.on( 'move', function () {
  ...
  reteSlider2.go(reteSlider1.index );
  reteSlider3.go(reteSlider1.index );
